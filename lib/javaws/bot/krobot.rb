...
require 'javaws/bot/krobot/behavior'
require 'javaws/bot/krobot/actions'
require 'javaws/bot/krobot/tasks'

class KRobot < Behavior
  include JAVAWS::BOT::ROBOT
  CONF_YAML = "krobot.yml"
  attr :username, true
  attr :password, true
  attr :browser,  true
  attr :config,   true
  ...
  def initialize(options = nil)
    ...
    @username = @config.read('krobot.username')
    @password = @config.read('krobot.password')
    @cachedir = @config.read('krobot.cachedir')
    @conds    = {:login? => false}
    @cache    = init({})
  ...
  def desc
    return @config.read('krobot.describe') || super
  end
end
