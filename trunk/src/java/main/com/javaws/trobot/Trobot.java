...
public class Trobot implements Runnable {
	...
	private static Log log = LogFactory.getLog(Trobot.class);
	...
	public boolean login() throws Exception {
		...
		try {
			...
			Document page_login = htmlUtils.getResponseDocument(form_login);
	...
	public InfoAllianz allianz(String uri) throws Exception {
		...
		try {
			...
			Document page_tbg = htmlUtils.getResponseDocument(table_tbg);
			...
			for (Element elem_tr : elem_trs) {
				...
				if (!"rbg".equals(elem_tr.attributeValue("class"))) {
					...
					infoAllianz.spielers.put(user_no, infoSpieler);
